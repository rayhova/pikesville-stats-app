...
export function ShotChartInput({
  markers,
  selection,
  onSelectLocation,
  large = false,
  ...
  onSelectLocation?: (selection: { x: number; y: number }) => void;
  large?: boolean;
}>) {
  const [internalSelection, setInternalSelection] = useState<{ x: number; y: number } | null>(null);
  ...
  return (
    <div className="shot-chart-input">
      {!onSelectLocation ? <input type="hidden" name="shotX" value={activeSelection?.x ?? ""} /> : null}
      {!onSelectLocation ? <input type="hidden" name="shotY" value={activeSelection?.y ?? ""} /> : null}
      <div
        className={`court-placeholder interactive-court ${large ? "large-court" : ""}`}
        onClick={(event) => {
          const target = event.target;

          if (
            target instanceof HTMLElement &&
            target.closest(".shot-marker, .shot-marker-popover")
          ) {
            return;
          }
          ...
          if (onSelectLocation) {
            onSelectLocation(nextSelection);
            return;
          }
